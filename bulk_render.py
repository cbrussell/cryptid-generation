...
def worker(start, stop):
    ...
    for id in range(start, stop):
        ...
        palette = f' ffmpeg -y -f image2 -framerate 24 -thread_queue_size 512 -i output/raw/{id}/{id}_%03d.png -vf palettegen=reserve_transparent=on output/raw/{id}/{id}_palette.png'
        subprocess.call(palette,shell=True)

        gif = f'ffmpeg -y -f image2 -framerate 24 -thread_queue_size 512 -i output/raw/{id}/{id}_%03d.png -i output/raw/{id}/{id}_palette.png -filter_complex "[0]scale=775:-1:flags=lanczos[j];[j][1]paletteuse=new=1"  output/gifs/{id}.gif'
        subprocess.call(gif,shell=True)

        png = f'ffmpeg -r 24 -y -thread_queue_size 512  -i output/raw/{id}/{id}_%03d.png  -filter_complex "scale=750:-1:flags=bicubic,setpts=PTS-STARTPTS" -plays 0   -f  apng   output/gifs/{id}.png'
        subprocess.call(png,shell=True)

        os.remove(f'output/raw/{id}/{id}_palette.png')
        number += 1
